...
**pre-processing**
 ...
 - load the data (t's in wide format to begin with)
...
**Analysis questions**
  ...
  - use dplyr to calculate the overall means for congruent vs. incongruent (note: need to get congruent and incongruent mean for each subject, and then get the mean of those means for congruent and incongruent)
  - use dplyr to calculate the standard error of the mean for congruent vs. incongruent (same requirement as above, first get the means for each subject, then get the standard error of those means)
...
1. Input the data: combine all of the individual text files into a single, long-format data frame
2. Pre-Processing & checks:
3. Analysis:
  a. What is the main effect of Congruency (congruentv. incongruent), output a table
  b. Display the means for the Congruency Factor in a graph using ggplot2, include the standard error of the means for the error bars
  c. Sequence Analysis, report the means in a table, show the means in a graph.
